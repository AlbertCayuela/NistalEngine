...
bool ModuleFS::AddPath(const char* path_or_zip)
{
	bool ret = false;
...
void ModuleFS::CreateDirectory(const char* directory)
{
	PHYSFS_mkdir(directory);
}

void ModuleFS::DiscoverFiles(const char* directory, vector<string>& file_list, vector<string>& dir_list) const
{
...
bool ModuleFS::CopyFromOutsideFS(const char* full_path, const char* destination)
{
...
bool ModuleFS::Copy(const char* source, const char* destination)
{
	bool ret = false;
...
void ModuleFS::NormalizePath(char* full_path) const
{
...
void ModuleFS::NormalizePath(std::string& full_path) const
{
...
unsigned int ModuleFS::Load(const char* path, const char* file, char** buffer) const
{
...
uint ModuleFS::Load(const char* file, char** buffer) const
{
	uint ret = 0;
...
void* ModuleFS::BassLoad(const char* file) const
{
	PHYSFS_file* fs_file = PHYSFS_openRead(file);
...
uint ModuleFS::Save(const char* file, const void* buffer, unsigned int size, bool append) const
{
	unsigned int ret = 0;
...
bool ModuleFS::SaveUnique(string& name, const void* buffer, uint size, const char* path, const char* prefix, const char* extension)
{
	char result[250];
...
bool ModuleFS::Remove(const char* file)
{
	bool ret = false;
...
const char* ModuleFS::GetBasePath() const
{
	return PHYSFS_getBaseDir();
}

const char* ModuleFS::GetWritePath() const
{
	return PHYSFS_getWriteDir();
}

const char* ModuleFS::GetReadPaths() const
{
